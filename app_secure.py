...
def get_vault_item(item_id):
    """
    ✅ ENDPOINT SEGURO
    Implementa validação de propriedade (Resource Ownership Check).
    Previne BOLA/IDOR.
    """
    token = request.headers.get('Authorization', '').replace('Bearer ', '')
    ...
    # Adicionamos "AND user_id = ?" na query SQL.
    # Isso garante que o banco só retorne o item se o ID bater E se pertencer ao usuário logado.
    cursor.execute('''
    ...
    if not item:
        # Retorna 404 tanto se o item não existir quanto se não for dele.
        # Isso evita "User Enumeration" ou vazamento de existência de recursos.
